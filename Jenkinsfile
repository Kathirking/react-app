pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("martin75/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'martin75') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull martin75/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d martin75/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
